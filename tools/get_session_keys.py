...
def main(argv: list[str] | None = None) -> int:
    ...
    print("\nThen run either:")
    print(
        "  python tools/get_session_keys.py --code "
        f"'<paste ssh-app redirect or code>' {code_verifier}"
    )
    print(
        "  python tools/get_session_keys.py --from-har capture.har "
        "--code-verifier <verifier> -o session_keys.json"
    )
    print("  python tools/get_session_keys.py --login --open -o session_keys.json")
    print(
        "  python tools/get_session_keys.py --refresh "
        "-i session_keys.json -o session_keys.json"
    )
    print("=" * 80)
    return 0
